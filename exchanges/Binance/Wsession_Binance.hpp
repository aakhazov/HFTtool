#ifndef _WSESSION_BINANCE_HPP_
#define _WSESSION_BINANCE_HPP_

#include "exchanges/base/Wsession.hpp"

#include "WstreamRequest_Binance.hpp"
#include "WstreamResponse_Binance.hpp"

class Wsession_Binance : public Wsession {
private:
	const uint8_t max_instruments_in_request_ = 30;
	std::chrono::duration<int64_t> timer_period_;
	int request_timer_ = 0;

	std::map<uint32_t, std::shared_ptr<WstreamRequest_Binance>> requests_;

public:
	explicit Wsession_Binance(net::io_context &ioc,
							  ssl::context &ctx,
