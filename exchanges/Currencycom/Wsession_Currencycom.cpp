...
void Wsession_Currencycom::on_timer(const boost::system::error_code &ec)
{
	if (ec)
	...
	if (--request_timer_ <= 0) {
		...
		// Periodic ping to avoid connection close due to 'End of file (2)' error
			...
			if (!is_in_responses) {
				...
				if (requests_.empty() ||
					...
					requests_.insert_or_assign(xmit_id_,
											   std::make_shared<WstreamRequest_Currencycom>(xmit_id_,
				...
				spdlog::trace("Request: Currencycom id = {}, '{}', {}",
							  xmit_id_,
	...
	if (requests_.size() > 0) {
		for (const auto &request : requests_) {
			if (request.second->get_size() > 0 &&
				...
				spdlog::trace("JSON tree as plain string: Currencycom '{}'", json);
				...
				for (const auto &instrument : request.second->get_conventional_responses()) {
					...
					spdlog::trace("Instrument: Currencycom '{}', '{}'", instrument.first, instrument.second);
				}
...
// Parse stream data
void Wsession_Currencycom::output(beast::flat_buffer &buffer)
{
	if (buffer.size() > 0) {
		// Convert received buffer to string
		std::string json_raw = beast::buffers_to_string(buffer.data());

		spdlog::trace("Parse stream data: Currencycom '{}'", json_raw);

		// Try parsing the string as a trade detail update message
		auto ticks = WstreamResponse_Currencycom::trade_detail(json_raw);
		if (ticks != nullptr && ticks->size() > 0) {
			...
			for (auto &tick : *ticks) {
				...
				if (is_in_reverse_conversion_table) {
					...
					spdlog::trace("Send price to analyzer: Currencycom {}, {}, {}, {}, {}",
								  field_symbol,
								  tick->get_timestamp(),
								  tick->get_price_str(),
								  tick->get_size_str(),
			...
			uint32_t id = WstreamResponse_Currencycom::response(json_raw);
			if (id != 0) {
				spdlog::trace("Response: Currencycom id = {}, '{}'", id, json_raw);
				...
				if (is_in_requests) {
					...
					for (const auto &instrument : request->second->get_conventional_responses()) {
						...
						spdlog::trace("Instrument: Currencycom '{}', '{}'", instrument.first, "success");
					}
				}
			}
			else
				spdlog::warn("Unknown response: Currencycom '{}'", json_raw);
		}
	}
